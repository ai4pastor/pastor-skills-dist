...
**구조까지 바꾼 완전한 내 스킬**을 갖고 싶으시면 이렇게 말씀해 보세요.

```text
묵상 스킬을 내 것으로 복사해서 kim-devotion 이라는 이름으로 만들어줘. 표지에 우리 교회 이름을 넣고 Day 6(토요일)도 추가해줘.
```

Claude가 스킬을 `~/.claude/skills/kim-devotion/` 으로 복제해 드리고, 이후 `/kim-devotion` 으로
부를 수 있습니다. 단, 복제본은 이후 업데이트(버그 수정·새 시안)를 받지 못합니다 —
취향 조정이 목적이면 나만의 규칙 파일을 먼저 권해 드립니다.
